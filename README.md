...
- `yx360 forms questions add <survey-id>` — добавление вопроса-оценки 1..N (с подтверждением).
...
`survey-id` передается явно: команды «список всех форм» нет — у API Форм нет документированного эндпоинта перечисления. `forms create` задает только заголовок; вопросы добавляются отдельно через `forms questions add` (сейчас тип — оценка 1..N, radio).
...
- `forms questions add` пока умеет только вопрос-оценку (radio 1..N); другие типы вопросов и удаление вопросов не реализованы.
