...
Current baseline: `yx360 login` works end-to-end through documented OAuth; Mail read/search/read-attachment/send works through IMAP/SMTP; Calendar CRUD via CalDAV and Telemost link creation are implemented and live-smoked. Forms read/create/add-questions/publish is live-verified through the documented Forms API against a real Yandex 360 for Business org (Forms API is Business-org-only). Headless manual login (`login --manual --begin/--complete`, `verification_code` redirect + PKCE) is build/unit/smoke-verified; live end-to-end pending B-1 (register the redirect URI in each OAuth app in the Yandex UI — OQ-018). Broader DoD remains: the agent skill drives the CLI, and `brew install` from the tap works.
