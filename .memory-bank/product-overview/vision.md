...
Current baseline: `yx360 login` works end-to-end through documented OAuth; Mail read/search/read-attachment/send works through IMAP/SMTP; Calendar CRUD via CalDAV and Telemost link creation are implemented and live-smoked; Forms read/create/add-questions/publish is live-verified through the documented Forms API against a real Yandex 360 for Business org (Forms API is Business-org-only); Disk list/get/put/share/unshare/rm/mkdir is **live-verified 2026-07-10** via the documented Yandex Disk REST API (`login --disk --manual --begin/--complete` → `disk list` confirmed on real account, B-1 resolved); **`--dry-run` persistent flag live-verified 2026-07-11** across disk/mail/calendar/telemost mutating commands (`--dry-run` overrides `--yes`; human `[dry-run]` + JSON `{"dry_run":"true","would":"..."}` output modes, exit 0; D-015); Headless manual login (`login --manual --begin/--complete`, `verification_code` redirect + PKCE) is build/unit/smoke-verified; live end-to-end pending OQ-018 (register the `verification_code` redirect URI in each OAuth app); **MCP stdio server (`yx360 mcp serve`) build-verified 2026-07-12** — 14 tools across Disk/Mail/Calendar/Telemost surfaces via `github.com/modelcontextprotocol/go-sdk` v1.6.1; `confirmed: bool` gate on all mutating tools; token sanitization on errors; live MCP client smoke pending (OQ-022; D-016). Broader DoD remains: the agent skill drives the CLI, and `brew install` from the tap works.
