...
| `--dry-run` | Persistent root-level flag that prints what would happen without executing any mutating command. Overrides `--yes` when both are supplied. Human output: `[dry-run] <message>`; JSON output: `{"dry_run":"true","would":"<message>"}`. Both exit 0. Silent no-op on read-only commands and `login`/`logout`. Deferred for `forms create/publish/unpublish` (OQ-021). See D-015. |
| `isDryRun()` | CLI-layer helper in `internal/cli/output.go` that returns the value of the `dryRun` package-level var. Checked at the top of each mutating `RunE` before any `!yes` gate or service call, so `--dry-run` always wins over `--yes` (D-015). |
| `emitDryRun(cmd, msg)` | CLI-layer helper in `internal/cli/output.go` that writes the dry-run output (human: `[dry-run] <msg>`; JSON: `{"dry_run":"true","would":"<msg>"}`) and returns nil (exit 0). Called immediately after `isDryRun()` returns true in each mutating `RunE` (D-015). |
| MCP (Model Context Protocol) | JSON-RPC 2.0 protocol for exposing tools to LLM clients such as Claude Desktop. `yx360 mcp serve` implements the stdio transport variant. |
| StdioTransport | MCP transport that communicates over stdin/stdout with newline-delimited JSON. Requires stdout to be clean (no cobra output). |
| confirmed parameter | Boolean MCP tool parameter required by all mutating tools (ANTI-2). Without `confirmed=true` the tool returns a `[dry-run]` preview instead of executing. |
