...
- `.Codex/agents/` with ≥10 agent files
- `.Codex/skills/setup/SKILL.md` (this file)
...
- `README.md`, `AGENTS.md`, `AGENTS.md` if they exist
...
- Whether `.memory-bank/` / `.assistant/` / `.Codex/` already exist (any of these = previous install or conflict)
- Existing `AGENTS.md` (back up to `Codex.local.md` if it exists and is non-trivial — i.e., not a stub)

### Step 3.5 — Mine prior AI-CLI sessions (parallel)

Before interviewing the user, mine the prior Codex / OpenCode / Codex sessions for the target project. They typically contain a lot of user context the user won't think to repeat in the install interview: domain glossary, tech-stack details, friction patterns, recurring complaints, open questions already voiced, validated approaches.

**Detect session directories.** Slugify the target path into the AI-CLI's session-dir convention:

- **Codex:** `~/.Codex/projects/<slug>/*.jsonl` — slug = target absolute path with `/` replaced by `-` (e.g., `/Users/ayusavin/Projects/jukte` → `-Users-ayusavin-Projects-jukte`).
...
> You are a session-miner for the Effective Harness `/setup` skill. Read the following Codex / OpenCode / Codex session JSONL files in `<dir>` (your assigned subset: `<file1>`, `<file2>`, ...). **Do NOT read full files** (they can be 10–100MB) — use `head -300` + `tail -300` + a mid-sample per file. Look for:
>
...
2. **Primary stack** (multiSelect) — backend / web frontend / iOS / Android / Flutter / infra. Used to pick executing-agent scope defaults in the target's `AGENTS.md`.
3. **One-line vision** — "What does this project do, in one sentence?" Seeds `.memory-bank/product-overview/vision.md`.
4. **Touch policy** — should `/setup` overwrite an existing `AGENTS.md` (back up as `Codex.local.md`) or refuse to overwrite (abort, ask user to merge manually)?
...
About to install harness <commit-sha> into <target-path>:
  ...
  Will create:
    .Codex/agents/                 (12 files)
    .Codex/hooks/inject-state.sh
    .Codex/skills/                 (7 skills: pre-feature, research, setup, anti-ai-slop-writing, frontend-design, swiftui-pro, swiftui-macos-26, memory-bank-defrag)
    .Codex/settings.json
    .assistant/INVARIANTS.md
    AGENTS.md
  ...
  Will create if missing (templates):
    ...
    AGENTS.md                       (short entry point)
    .harness-lock                   (version metadata)

  Will skip / preserve:
    Any file the project already owns and Touch policy says skip
    Existing AGENTS.md → backed up to Codex.local.md if Touch policy says overwrite
...
```json
{
  ...
  "files": {
    ".Codex/agents/architect.md": { "owner": "harness", "sha256": "<computed>" },
    ".Codex/hooks/inject-state.sh": { "owner": "harness", "sha256": "<computed>" },
    ".memory-bank/product-overview/vision.md": { "owner": "project-template", "sha256": "<computed>" },
    "AGENTS.md": { "owner": "project-template", "sha256": "<computed>" },
    ...
  }
}
```
...
**`<target>/AGENTS.md`**: a short entry point — points at `AGENTS.md`, `.memory-bank/index.md`, declares `PROJECT_TYPE`, declares stack. Backs up any existing `AGENTS.md` to `Codex.local.md` if Touch policy = overwrite.
...
- `.Codex/hooks/inject-state.sh` is executable (`chmod +x` if not)
- `.harness-lock` parses as JSON
- `.assistant/INVARIANTS.md` exists and is non-empty
- `AGENTS.md` exists and is non-empty
- The SessionStart hook script runs without error: `bash <target>/.Codex/hooks/inject-state.sh` → exit 0
...
Created:
  - 12 agent files under .Codex/agents/
  - 8 skills under .Codex/skills/ (including /pre-feature, /research, /setup)
  - SessionStart hook
...
Next steps:
  1. cd <target-path>
  2. Open the project in Codex / OpenCode / Codex
...
- **Half-installed state.** If `.Codex/` exists but `.harness-lock` is missing → ask the user to either clean up manually or accept the Touch policy and proceed.
...
### Flow B — existing project, with existing AGENTS.md
...
[skill detects existing AGENTS.md]
[asks Touch policy → user picks "back up to Codex.local.md and overwrite"]
[plan + confirm + copy + seed]
[user manually merges any custom rules from Codex.local.md into the new AGENTS.md afterwards]
```
