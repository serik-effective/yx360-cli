...
2. **Auto-memory** — the cross-session memory at
   `~/.Codex/projects/<slugified-cwd>/memory/` (an `MEMORY.md` index plus one
...
3. **Repo current-state docs** — files that assert *what is true now* and drift
   the same way: `README.md`, `AGENTS.md`/`AGENTS.md`, design specs, proposals.
...
- If the project's `AGENTS.md` defines a memory-update protocol (e.g. an
