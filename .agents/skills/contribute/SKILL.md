...
**Skill** → `.Codex/skills/<name>/SKILL.md`: frontmatter (`name`, `description`) + body matching the house style (see `pre-feature/SKILL.md` for an orchestrator, `quickstart/SKILL.md` for a single-pass tool). Include When-to-invoke / When-NOT, Invocation, and the workflow.
...
5. **No collision** — `<name>` not already taken in `.Codex/agents/` or `.Codex/skills/`.
6. **Routing** — if a new keyword→role row belongs in root `AGENTS.md` "Profile keywords → roles", add it in the same branch.

### Step 7 — Commit, push, open PR

```
git add .Codex/agents/<name>.md   # or skills/<name>/, + AGENTS.md if routing changed
