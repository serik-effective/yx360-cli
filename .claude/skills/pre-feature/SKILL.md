...
- Explicit reminder: "Output strict YAML per `.claude/agents/<role>.md` schema. No prose. Cite file:line for every finding. Empty array allowed."

The four subagents:
1. `architect` — read `.claude/agents/architect.md`, focus on module boundaries, pipeline stage fit, SOLID, migration path
2. `skeptic` — read `.claude/agents/skeptic.md`, find flaws / hidden costs / scope creep / invariant violations
3. `researcher` — read `.claude/agents/researcher.md`, find external prior art and best practices via `mcp-omnisearch`. Confidence flags mandatory.
4. `reviewer` — read `.claude/agents/reviewer.md`, cross-check against INVARIANTS / anti-stories / prior decisions
