...
Default crew (skip any whose `.claude/agents/<role>.md` doesn't exist in the project):
...
- Hard contract: "Output strict YAML per `.claude/agents/<role>.md`. Severity-tag every finding (HIGH/MEDIUM/LOW). Cite file:line. No prose. No praise."
...
/audit branch
/audit pr 142
/audit path .claude/agents/scraping-* --severity HIGH
```
